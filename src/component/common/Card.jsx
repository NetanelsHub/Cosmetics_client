...
export default function Card() {
    const { productsByCategory, showModel, setShowModel, selectedProduct, setSelectedProduct } = useContext(globalContext);
    ...
    return (
        ...
        <div className="flex flex-wrap justify-center">
            ...
            {!productsByCategory ? 
            (<div className="flex justify-center items-center" style={spinnerStyles}></div>) : (
                productsByCategory.map((product, index) => (
                    <div key={index} className="w-full mt-5 max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 m-4">
                        ...
                        <div className="px-5 pb-5">
                            <h5 className="text-xl font-semibold tracking-tight text-customGold dark:text-customGold">{product.product_name}</h5>
                ...
                <Model onClick={handleCloseModel}>
                    <div className='flex items-center'>
                        ...
                        <div className='flex flex-col ml-4'>
                            <h2 className="text-3xl font-bold mb-4 text-customGold">{selectedProduct.product_name}</h2>
                            <p className="text-lg mb-4">{selectedProduct.product_description}</p>
                            <span className="text-3xl font-bold">${selectedProduct.product_price}</span>
                        </div>
                    </div>
